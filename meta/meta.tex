...
\begin{dfn}
    We may construct uniform substitution $[n = m(j \setminus i)]_s$ by means of a formation sequence.
    \begin{align*}
...
Note the peculiar order of formation whereby $n$ is base and $m$ final.
...
\begin{dfn}
    ...
    \begin{description}[
        ...
        \item[predicative formula] $\pred^{\rho, \delta}(n) \equiv \mathord\exists_{\iota = 1}^\delta i_\iota \leq n\; \&_{\iota = 1}^\delta \const(i_\iota)
                  \\\conj n = \ord\rho * \ord( * (*_{\iota = 1}^\delta i_\iota * (\iota \neq \delta \mathrel{\&\&} \ord{,})) * \ord)$
    \end{description}
    Then there is a formation sequence for well formed formulas $[\mathrm{wff}(n)]_s$.
    \begin{align*}
...
\begin{dfn}
    We may construct exponentiation by means of a formation sequence $[b^m = n]_s^n$.
    \begin{align*}
        \mathrm{final}(\sigma, p, f)     \equiv\;       & f = m *_p \sigma *_p n                                              \\
...
\newcommand{\diag}{\mathrel{\mathrm{diag}}}

\begin{dfn}
    Ordinal.

    $[\varphi(\overline n)]_s \equiv \exis{m < s}\exis{k < m}\exis{i < k} [k = p^n]_s \conj i \monos \ord{'}\\
        \conj m = \ord{\mathrm c} \times k + i \conj \varphi(m)$

    Diagonal.
    $$[d \mathrel{\mathrm{diag}} n]_s \equiv [d = n(\overline n \setminus x)]_s$$

    Any expression from the numerised logical alphabet corresponds to a  number $\nu$.
    $$\ord{d \diag x} \equiv \suc^\nu(0)$$
\end{dfn}

\begin{lmm}
    $$d = \ord{d \diag \ord{d \diag x}} \eqv d \diag \ord{d \diag x}$$
    $$d = \ord{\neg(d \diag \ord{\neg(d \diag x)})} \eqv d \diag \ord{\neg(d \diag x)}$$
\end{lmm}

\begin{dfn}
    $[\mathrm{pinstance}(n)]_s$
    \begin{align*}
        \mathrm{final}(f) \equiv\;              & \mathrm{paxiom}(f)                                                              \\
        \mathrm{base}(h, n) \equiv\;            & h = n \conj \univ{i \leq h} i \subs h \then \neg(\ord{\mathrm p} \fuck i)       \\
        \mathrm{case}(h, i, k, l) \equiv\;      & h = k * \ord {\mathrm p} * i * l \conj i \monos \ord' \conj \neg(\ord' \fuck l) \\
        \mathrm{formation}(i, k, l, g) \equiv\; & \exis{f < g} \mathrm{wff}(f) \conj g = h(f \setminus \ord {\mathrm p} * i)
    \end{align*}
    \begin{align*}
        [\mathrm{axiom}(n)]_s \equiv\; &
    \end{align*}
    $[\mathrm{theorem}(n)]_s$
    \begin{align*}
        \mathrm{case}(q)                            \equiv\; & q = q                                                 \\
        \mathrm{formation}(p, r) \equiv\;                    & r = \ord{\rightarrow} * \ord( * p * \ord, * q * \ord)
    \end{align*}
\end{dfn}

\begin{thm}
    \begin{equation*}
        \mathcal Q \infers S \text{\;only if\;} \mathcal Q \infers \mathrm{theorem}(\ord{S})
    \end{equation*}
\end{thm}

\begin{dfn}
    $$
        G(x) \equiv \exis g  g \diag x \conj \neg \mathrm{theorem}(g)
    $$
\end{dfn}
