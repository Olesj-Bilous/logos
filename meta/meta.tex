...
The positive domain $v^{-1}(1) \subseteq \wffs$ of a valuation map $v$ contains all propositions $P$ for which $v(P) = 1$. We can see that a valuation map is inferential only if the positive domain is deductively closed. A model $v$ supports $(\mathcal E, P)$ only if $P \in v^{-1}(1)$ if $\mathcal E \subseteq v^{-1}(1)$
...
Note that each deductively closed set which is maximally consistent is a model set if our logic permits it.

\begin{lmm}
    A model set is complete and decidable.
    \begin{proof}
...
This suggests an extension of paraconsistent semantics with a decidability clause. In any case, a model set aptly determines a model.
...
\begin{thm}
    ...
    The semantics are characteristic for the inferential system. The converse means that the inferential system is complete with respect to the semantics.
\end{thm}
