...
\begin{dfn}
    A model $v \in \mathbf M$ contains a set of assignment maps such that for each $n \in \mathbb N$:
    \begin{enumerate}
        \item $v(x^n) \in \mathrm U^n$ for each $x^n \in \mathbf x^n$
        \item $v(P^n) \in \wp(\mathrm U^n)$ for each $P^n \in \mathbf P^n$
    \end{enumerate}
    Furthermore, it contains a complete, not trivial inferential map $v: \Phi \longrightarrow \setsm{0,1}$ such that:
    \begin{enumerate}
        \item $v(P^n(x^n)) = 1$ only if $v(x^n) \in v(P^n)$
        \item $v(\univ x \varphi(x)) = 1$ only if $w(\varphi(x)) = 1$ for all $w \in \mathbf M$ such that $w(y) = v(y)$ for all $y \in \mathbf x$ where $y \neq x$
    \end{enumerate}
