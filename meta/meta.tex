...
\begin{dfn}
    A logical operator $o$ of order $n \in \mathbb N$ may act on any $n$-tuple of propositions $P^n \subseteq \mathcal W$ to compose a proposition $o(P^n) = P \in \mathcal W$.

    Each $P_i \in P^n$ is said to be the $i$th direct descendant of $P$.

    Note that each component $P^i \in P^n$ of a composite proposition $P = o(P^n)$ may itself be composite. A proposition $Q$ is said to be a descendant of $P$ if there is a succession of direct descendants that leads from $P$ to $Q$.

    When a proposition $P$ is atomic there is no logical operator $o$ of any order $n$ nor any $n$-tuple $P^n \subset \wffs$ of which it is composed $P = o(P^n)$.
\end{dfn}

\begin{dfn}
    Given any alphabet consisting of a finite set of symbols, we consider any finite enumeration of symbols from the alphabet an expression.
    ...
    We may use an operator symbol $\mathrm o$ and the succession symbol $'$ to denote any operator. Then parentheses $($, $)$ and a comma $,$ allow us to denote components in a composite proposition.
\end{dfn}

\begin{thm}
    The set of all propositions $\mathcal W$ may be enumerated though without bound: $|\wffs| = |\mathbb N|$.
    \begin{proof}
        The alphabet $\setsm{\mathrm a, \mathrm o, (, ),\,,}$ allows any proposition to be encoded in base $5$.
    \end{proof}
\end{thm}
